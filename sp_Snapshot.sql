...
Author: David Fowler  
Revision date: 30 September 2025
Version: 3  
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            SET @SQL = 'USE ' + @Databases + 'DECLARE @DatabaseName VARCHAR(128)  
                ...
                SELECT name, physical_name, REVERSE(SUBSTRING(REVERSE(Physical_name),CHARINDEX(''.'', REVERSE(physical_name),0) +1,LEN(physical_name)))  
                FROM sys.master_files  
                WHERE type != 1 AND database_id = DB_ID()  
    
                --begin building snapshot script  
                DECLARE @SnapshotScript VARCHAR(2000)  
SET @SnapshotScript = ''CREATE DATABASE '' + QUOTENAME(@SnapshotName) + '' ON ''  
                ...
                BEGIN  
                    ...
                    IF ''' + @FilePath +   ''' = ''''  
                    BEGIN  
                        SELECT @SnapshotScript = @SnapshotScript + ''(NAME = '' + QUOTENAME(name) + '', FILENAME = '''''' + ss_file_name + ''_'' + @SnapshotName + ''.ss''''),''  
                        FROM @DatabaseFiles  
                        WHERE id = @LoopCounter  
                    END  
                    ELSE  
                    BEGIN  
                        SELECT @SnapshotScript = @SnapshotScript + ''(NAME = '' + QUOTENAME(name) + '', FILENAME = ''''' + @FilePath + ''' + ''_'' + @SnapshotName + ''.ss''''),''  
                        FROM @DatabaseFiles  
                        WHERE id = @LoopCounter  
                    END  
                END  
				...
				IF (@STMTOnly = 0)
					SET @SQL += 'EXEC (@SnapshotScript)'
				ELSE 
					SET @SQL += 'PRINT @SnapshotScript  '
